...
class Interception {
  ...
  reinitializeLabels(showNumberIndicators = true) {
    if (this.APCmini) {
      this.APCmini.forEach((i) =>
        this.#updateLabelFor(i, showNumberIndicators)
      );
    }
  }
  ...
  #updateLabelFor(nnote, showNumberIndicators = false) {
    ...
    DOMElement.textContent =
      this.defaultLooksMap.get(nnote)?.label ??
      (showNumberIndicators ? nnote : "");
  ...
  updateLook(nnote, newlook) {
    ...
    this.defaultLooksMap.set(nnote, {
      ...this.defaultLooksMap.get(nnote),
      ...newlook,
    });
    if (this.APCmini) {
  ...
  lockDesk() {
    ...
    this.APCmini.displayTextAnimation(
      this.defaultLooksMap.get(512)?.text ?? defaultLockAnimation.text,
      this.defaultLooksMap.get(512)?.speed ?? defaultLockAnimation.speed,
      true,
      undefined,
      undefined,
      8,
      (idx, on) => {
        console.log();
        this.APCmini.changeButton(
          idx,
          on
            ? this.defaultLooksMap.get(512)?.on?.color ||
                defaultLockAnimation.on.color
            : this.defaultLooksMap.get(512)?.off?.color ||
                defaultLockAnimation.off.color,
          on
            ? this.defaultLooksMap.get(512)?.on?.lmode ||
                defaultLockAnimation.on.lmode
            : this.defaultLooksMap.get(512)?.off?.lmode ||
                defaultLockAnimation.off.lmode
        );
      }
    );
  }
