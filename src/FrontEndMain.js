...
window.toFrontEnd.onMenuClick((data, params) => {
  switch (data) {
    case "newProject":
      NewEmptyProjectButtonClick();
      break;
    case "openFile":
      loadConfigButtonClick();
      break;
    case "save":
      saveButtonClick();
      break;
    ...
    case "printConsole":
      printConsole(params);
      break;
    case "toggleEditLockStatusSection":
...
function connectToMIDIDevices() {
  ...
  interception
    ...
    .then((res) => interception.intersept((msg) => {}));
}
...
function selectNote(nnote, element) {
  ...
  const selectedCells = document.getElementsByClassName(
    "selectedCellsContainer"
  );

  const newElement = document.createElement("div");
  newElement.innerText = nnote;
  newElement.classList.add("selectedCells");
  newElement.id = "selectedCell" + nnote;

  const newElement2 = document.createElement("div");
  newElement2.innerText = nnote;
  newElement2.classList.add("selectedCells");
  newElement2.id = "2selectedCell" + nnote;

  selectedCells.item(0).appendChild(newElement);
  selectedCells.item(1).appendChild(newElement2);
...
function unselectNote(nnoteID, element) {
  ...
  const toRemove = document.getElementById("selectedCell" + nnoteID);
  const toRemove2 = document.getElementById("2selectedCell" + nnoteID);
  selectedCells.item(0).removeChild(toRemove);
  selectedCells.item(1).removeChild(toRemove2);
...
function updateNewEmptyProjectButtonState() {
  ...
  const newEmptyProjectButton = document.getElementById(
    "NewEmptyProjectButton"
  );
  const ButtonDisabledState =
    interception.defaultLooksMap.size === 0 ||
    interception.canOverwrite() === false;
...
function updateSaveButtonState() {
  ...
  const ButtonEnabledState =
    (projectFilePath && !interception.canOverwrite()) == true;
...
function updateLook() {
  ...
  const shouldOverwrite = document.getElementById(
    "OverwriteLabelCheckbox"
  ).checked;
...
function UpdateLockStatusButtonClicked() {
  if (interception.isDeskLocked()) return;

  const animationText = document.getElementById("AnimationTextInput").value;
  const animationSpeed = parseInt(
    document.getElementById("AnimationSpeedInput").value
  );
...
function fillInitialLockStatus() {
  const animationText = document.getElementById("AnimationTextInput");
  const animationSpeed = document.getElementById("AnimationSpeedInput");
  ...
  backgroundcolor.value =
    requestedData?.off?.color?.hex ?? defaultLockAnimation.off.color.hex;
  backgroundMode.selectedIndex =
    requestedData?.off?.lmode ?? defaultLockAnimation.off.lmode;
  foregroundcolor.value =
    requestedData?.on?.color?.hex ?? defaultLockAnimation.on.color.hex;
  foregroundMode.selectedIndex =
    requestedData?.on?.lmode ?? defaultLockAnimation.on.lmode;
}
...
async function saveButtonClick() {
  ...
  const result = await window.electronAPI.writeFile(
    projectFilePath,
    JSON.stringify(MapToJSON(interception.defaultLooksMap))
  );
...
async function loadConfigButtonClick() {
  if (!interception.canOverwrite()) {
    alert(
      "Please save your current config before importing a new one. Otherwise your work will be gone forever."
    );
    return;
  }
  ...
  if (!result.canceled) {
    ...
    const readResult = await window.electronAPI
      .readFile(filePath)
      .then((res) => {
        if (res.success) {
          //TODO: Hier kommt die Datei rein, aufteilung in ButtonData und ConfigData ne gute Idee
...
function PresettingsconnectButtonClick() {
  ...
  if (
    !(
      selected1.value != "Bitte wählen" &&
      selected2.value != "Bitte wählen" &&
      selected3.value != "Bitte wählen" &&
      selected4.value != "Bitte wählen" &&
      new Set([
        selected1.value,
        selected2.value,
        selected3.value,
    ...
    alert(
      "The current MIDI Selection is not valid. Check that the program 'loopMIDI' is running and that the MIDI Device is connected!"
    );
    return;
  }
  ...
  fillInitialLockStatus();
  document
    .getElementById("bottomPart")
    .removeChild(document.getElementById("PrestartSettings"));
...
function connectButtonClick() {
  ...
  document
    ...
    .removeChild(document.getElementById("MidiSettings"));
  document.getElementById("Pult").style.display = "block";
  document.getElementById("Settings").style.display = "block";
}
...
function printConsole(showNumbers) {
  ...
  const console_background = body.style.getPropertyValue(
    "--console-background"
  );
  const console_lighter_background = body.style.getPropertyValue(
    "--console-lighter-background"
  );
  const lowerBrightness = body.style.getPropertyValue(
    "--lower-brightness-darkening-effect-percent"
  );
  const console_button_font_size = body.style.getPropertyValue(
    "--console-button-font-size"
  );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  backgroundcolor.onchange = (i) => {
    i.target.value = findClosestColor(i.target.value).hex;
  };
  foregroundcolor.onchange = (i) => {
    i.target.value = findClosestColor(i.target.value).hex;
  };
  ...
  const UpdateLockStatusButton = document.getElementById(
    "UpdateLockStatusButton"
  );
  UpdateLockStatusButton.onclick = UpdateLockStatusButtonClicked;
